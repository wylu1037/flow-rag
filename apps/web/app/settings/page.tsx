...
export default function SettingsPage() {
  return (
    <AppFrame>
      <Card>
        <CardHeader>
          <Badge variant="secondary" className="w-fit font-mono uppercase">Settings</Badge>
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                The current MVP uses a deterministic local answerer until provider keys are configured.
              </p>
            </div>
...
function RuntimeItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-border bg-background p-4">
      ...
      <p className="font-display mt-2 text-[28px] leading-tight tracking-[-0.03em] text-ink">{value}</p>
    </div>
  );
}
